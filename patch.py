...
with open(r'C:\Users\vardh\OneDrive\Documents\repos\GaneshaCelebrations\src\App.jsx', 'r', encoding='utf-8') as f:
    content = f.read()

# Block 1
b1 = '''import React, { useState, useEffect } from 'react';
import { Menu, X, ArrowRight, Calendar, MapPin, Music, Phone, MessageCircle, Info, Trash2, Edit2, Gift, Hand } from 'lucide-react';
import './App.css';'''

r1 = '''import React, { useState, useEffect } from 'react';
import { Menu, X, ArrowRight, Calendar, MapPin, Music, Phone, MessageCircle, Info, Trash2, Edit2, Gift, Hand } from 'lucide-react';
...
# Block 1a
b1a = '''  const [poojaEnrollments, setPoojaEnrollments] = useState(() => {
...
r1a = '''  const [poojaEnrollments, setPoojaEnrollments] = useState([]);
  const [prasadamEnrollments, setPrasadamEnrollments] = useState([]);'''
...
r3 = '''  const handleEnrollSubmit = async (e) => {
    e.preventDefault();
    try {
      if (enrollType === 'pooja') {
        if (formData.dates.length === 0) {
...
# Block 4
b4 = '''  const handleDeletePrasadamUser = (id) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
...
r4 = '''  const handleDeletePrasadamUser = async (id) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
...
with open(r'C:\Users\vardh\OneDrive\Documents\repos\GaneshaCelebrations\src\App.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
